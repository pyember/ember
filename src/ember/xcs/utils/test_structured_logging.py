...
class TestStructuredLogging(unittest.TestCase):
    ...
    def test_time_operation_decorator(self, mock_perf_counter):
        ...
        mock_perf_counter.side_effect = [0.0, 0.05, 0.1]  # Start, timestamp in log, end = 100ms duration
        ...
        mock_perf_counter.side_effect = [0.0, 0.005, 0.01]  # Start, timestamp in log, end = 10ms (below threshold)
    ...
    def test_enrich_exception(self):
        ...
        self.assertEqual(error.diagnostic_context, {}, "Initial context should be empty")
        
        # Enrich the exception with context
        enriched = enrich_exception(error, extra_context="value")
        
        # Verify explicit context was added
        self.assertEqual(enriched.diagnostic_context.get("extra_context"), "value", 
                        f"Explicit context missing: {enriched.diagnostic_context}")
        
        # Verify thread-local context was added
        self.assertEqual(enriched.diagnostic_context.get("operation_id"), "test_op", 
                        f"Thread context missing: {enriched.diagnostic_context}")
        self.assertEqual(enriched.diagnostic_context.get("node_id"), "test_node", 
                        f"Thread context missing: {enriched.diagnostic_context}")
...
def test_time_operation_decorator():
    ...
    with patch('time.perf_counter') as mock_perf_counter:
        ...
        mock_perf_counter.side_effect = [0.0, 0.05, 0.1, 0.0, 0.005, 0.01]  # Three values per call, two calls
