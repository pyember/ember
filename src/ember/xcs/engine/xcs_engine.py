...
from typing import Any, Dict, List, Optional, Set, Callable, Union, Tuple, TypeVar, Protocol, NamedTuple
import dataclasses
...
class TopologicalSchedulerWithParallelDispatch(TopologicalScheduler):
    ...
    def run_plan(self, *, plan: XCSPlan, global_input: Dict[str, Any], graph: XCSGraph) -> Dict[str, Any]:
...
def execute_graph(
    ...
    for wave in waves:
        ...
        if isinstance(scheduler, TopologicalScheduler) and not isinstance(scheduler, TopologicalSchedulerWithParallelDispatch):
            for node_id in wave:
                node = graph.nodes[node_id]
        ...
        # Parallel execution for parallel scheduler
            ...
            with ThreadPoolExecutor(max_workers=getattr(scheduler, 'max_workers', None)) as executor:
                futures = {}
