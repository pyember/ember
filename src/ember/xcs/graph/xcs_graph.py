...
class XCSGraph:
    ...
    def topological_sort(self) -> List[str]:
        ...
        in_degree = {node_id: len(node.inbound_edges) for node_id, node in self.nodes.items()}
    ...
    def __str__(self) -> str:
        ...
        """
        nodes_str = [f"Node {node_id}: {node.name or 'unnamed'}" for node_id, node in self.nodes.items()]
        edges_str = []
        for node_id, node in self.nodes.items():
            for edge in node.outbound_edges:
                edges_str.append(f"{node_id} -> {edge}")
                
        return f"XCSGraph with {len(self.nodes)} nodes:\n" + "\n".join(nodes_str) + "\n\nEdges:\n" + "\n".join(edges_str)
