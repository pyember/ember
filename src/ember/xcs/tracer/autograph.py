...
class AutoGraphBuilder:
    ...
    def build_graph(self, records: List[TraceRecord]) -> XCSGraph:
        ...
        for idx, record in enumerate(records):
            ...
            graph.add_node(
                operator=create_callable(record.outputs),
                node_id=node_id
            )
    ...
    def _analyze_dependencies(self, records: List[TraceRecord]) -> None:
        ...
        for j, record in enumerate(records):
            ...
            for i in range(j):
                ...
                if self._is_parent_child_relationship(record.node_id, records[i].node_id, hierarchy_map):
                    continue
    ...
    def _has_dependency(self, inputs: Dict[str, Any], outputs: Any) -> bool:
        ...
        if isinstance(outputs, dict):
            ...
            for input_key, input_value in inputs.items():
                if any(output_value == input_value for _, output_value in outputs.items()):
                    return True
