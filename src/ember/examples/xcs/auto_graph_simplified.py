...
def main() -> None:
    ...
    for i, input_data in enumerate(inputs):
        ...
        print(f"Computation: {input_data.value} + 10 = {input_data.value + 10}, then × 2 = {(input_data.value + 10) * 2}")
    ...
    with execution_options(scheduler="sequential"):
        start_time = time.time()
        result = pipeline(inputs=AdditionInput(value=20))  # Should result in (20+10)*2 = 60
        elapsed = time.time() - start_time
        print(f"Result: {result}")
        print(f"Value from result: {result.value}")
        # Show computation steps
