...
class ReasoningInput(EmberModel):
    ...
    context: Optional[str] = Field(
        default=None, 
        description="Optional context to provide background knowledge"
    )
...
class ReasoningOutput(EmberModel):
    ...
    confidence: float = Field(description="Overall confidence in the answer (0-1)", ge=0, le=1)
...
class ReasoningEnsemble(Operator[ReasoningInput, Dict[str, Any]]):
    ...
    def forward(self, *, inputs: ReasoningInput) -> Dict[str, Any]:
        ...
        for response in ensemble_results.get("responses", []):
            ...
            reasoning_paths.append(ReasoningSteps(
                steps=steps,
...
class ReasoningSynthesizer(Operator[Dict[str, Any], ReasoningOutput]):
    ...
    def forward(self, *, inputs: Dict[str, Any]) -> ReasoningOutput:
        ...
        overall_confidence = sum(verification_scores) / len(verification_scores) if verification_scores else 0.0
...
def main() -> None:
    ...
    with execution_options(max_workers=4):
        result = reasoning_system(inputs={
            "query": query,
            "context": context
        })
    ...
    print("2. ReasoningVerifier: Checks each reasoning path for accuracy, coherence, completeness")
