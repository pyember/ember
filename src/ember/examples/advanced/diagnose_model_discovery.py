...
def check_provider_registry():
    ...
    try:
        ...
        if not PROVIDER_REGISTRY:
            ...
            try:
                from ember.core.registry.model.providers import anthropic_provider, openai_provider
                ...
                for name, module in [("Anthropic", anthropic_provider), ("OpenAI", openai_provider)]:
                    for item_name in dir(module):
                        item = getattr(module, item_name)
                        if inspect.isclass(item):
                            logger.debug(f"Found class {item_name} in {name} provider")
                            if hasattr(item, "__provider_name__"):
                                logger.debug(f"Class {item_name} has __provider_name__ = {getattr(item, '__provider_name__')}")
            except ImportError as e:
                logger.error(f"Failed to import provider modules: {e}")
        else:
            logger.info(f"✅ PROVIDER_REGISTRY contains {len(PROVIDER_REGISTRY)} providers")
    ...
    for path in import_paths:
        try:
            ...
            if "discovery" in path:
                for item_name in dir(module):
                    item = getattr(module, item_name)
                    if inspect.isclass(item) and issubclass(item, BaseDiscoveryProvider):
...
def test_individual_discovery():
    ...
    discovery_classes = [
        ("ember.core.registry.model.providers.anthropic.anthropic_discovery", "AnthropicDiscovery"),
        ("ember.core.registry.model.providers.openai.openai_discovery", "OpenAIDiscovery")
    ]
            ...
            if discovery_class:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(f"❌ Error calling fetch_models() on {class_name}: {e}")
                except Exception as e:
...
def check_model_registry_implementation():
    ...
    try:
        ...
        if hasattr(discover_method, "__code__"):
            ...
            logger.debug(f"discover_models source (line {line_no}):\n{''.join(source_lines)}")
        
        # Check the factory implementation
        from ember.core.registry.model.base.registry.factory import create_provider_instance
        ...
        if hasattr(factory_method, "__code__"):
            ...
            logger.debug(f"create_provider_instance source (line {line_no}):\n{''.join(source_lines)}")
    except Exception as e:
...
def main():
    ...
    logger.info("\n===== DIAGNOSTIC SUMMARY =====")
    logger.info("Review the log output above to identify where the model discovery process is failing.")
