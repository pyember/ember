...
if model_ids:
    logger.info(f"Successfully discovered {len(model_ids)} models during initialization: {model_ids}")
else:
    logger.info("No models discovered during initialization, attempting manual discovery...")
    
    # Try explicit discovery
    discovered_models = registry.discover_models()
    
    if discovered_models:
        logger.info(f"Manual discovery found {len(discovered_models)} models: {discovered_models}")
    else:
...
def register_openai_models():
        ...
        model_infos = [
            ...
            models.ModelInfo(
                ...
                context_window=128000,
                cost=models.ModelCost(
                    input_cost_per_thousand=0.00015,
                    output_cost_per_thousand=0.0006
                ),
                rate_limit=models.RateLimit(
                    tokens_per_minute=10000000,
                    requests_per_minute=1500
    ...
    def register_anthropic_models():
        ...
        if not anthropic_key:
            logger.warning("ANTHROPIC_API_KEY not set, skipping Anthropic model registration")
            return []
        ...
        model_infos = [
            ...
            models.ModelInfo(
                ...
                context_window=200000,
                cost=models.ModelCost(
                    input_cost_per_thousand=0.015,
                    output_cost_per_thousand=0.075
                ),
                rate_limit=models.RateLimit(
                    tokens_per_minute=5000000,
                    requests_per_minute=1000
...
def check_specific_models(model_ids: List[str]):
    ...
    for model_id in model_ids:
        ...
        if exists:
            ...
            print(f"   - Provider: {info.provider.name if hasattr(info.provider, 'name') else 'Unknown'}")
...
def main():
    ...
    print("               model_service.invoke_model('openai:gpt-4o', 'What is the capital of France?')")
