...
# Replace direct API import with core import
...
def test_auto_discovery():
    ...
    if model_ids_1:
        logger.info(f"Registry without auto-discovery found {len(model_ids_1)} models: {model_ids_1}")
    else:
    ...
    if model_ids_2:
        logger.info(f"Registry with force_discovery found {len(model_ids_2)} models: {model_ids_2}")
    else:
    ...
    if model_ids_3:
        logger.info(f"Registry with auto_discover found {len(model_ids_3)} models: {model_ids_3}")
    else:
    ...
    for model_id in best_registry.list_models():
        ...
        if model_info:
            provider_name = model_info.provider.name if hasattr(model_info.provider, 'name') else 'Unknown'
            context_window = model_info.context_window if hasattr(model_info, 'context_window') else 'N/A'
            cost = model_info.cost if hasattr(model_info, 'cost') else None
            
            if cost:
                i_cost = f"${cost.input_cost_per_thousand:.5f}/1K" if hasattr(cost, 'input_cost_per_thousand') else 'N/A'
                o_cost = f"${cost.output_cost_per_thousand:.5f}/1K" if hasattr(cost, 'output_cost_per_thousand') else 'N/A'
                cost_str = f"(In: {i_cost}, Out: {o_cost})"
            else:
                cost_str = "(Cost: N/A)"
                
            logger.info(f"  - {model_id}: Provider={provider_name}, Context={context_window} {cost_str}")
