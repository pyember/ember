...
from ember.xcs.engine.xcs_engine import compile_graph, TopologicalSchedulerWithParallelDispatch
...
def demo_parallel_execution(*, num_ops: int, delay: float) -> None:
    ...
    print(f"\n{'=' * 80}")
    print(f"DEMO 2: Sequential vs Parallel Execution - {num_ops} operators, {delay:.3f}s delay each")
    print(f"{'=' * 80}")
    print("This demo shows how JIT + parallel scheduling drastically improves performance.")
    ...
    _ = seq_scheduler.run_plan(
        plan=plan, 
        global_input={"task_id": "sequential"}, 
        graph=graph
    )
...
def demo_jit_with_concurrent_execution(*, num_ops: int, delay: float) -> None:
    ...
    print(f"\n{'=' * 80}")
    print(f"DEMO 3: JIT with Concurrent Execution - {num_ops} operators, {delay:.3f}s delay each")
    print(f"{'=' * 80}")
    ...
    class ConcurrentJITOperator(Operator):
        ...
        def forward(self, *, inputs: DelayInput) -> EnsembleOutput:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_ops) as executor:
