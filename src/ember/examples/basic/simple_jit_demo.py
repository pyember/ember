...
def run_benchmark(
    *, 
    ...
    for i in range(num_runs):
        ...
        logger.info("    Completed %d operations in %.4fs", 
    ...
    *,
    ...
    if len(times) > 1:
        ...
        if speedup > 1.05:  # Only reporting meaningful speedups
            ...
            logger.info("  JIT caching benefit:  %.1f%% faster after tracing", 
                       improvement * 100)
    ...
    if baseline_time is not None and baseline_time > 0:
        ...
        if speedup > 1.05:  # Only reporting meaningful speedups
            ...
            logger.info("  Speedup vs baseline: %.1f%% faster (%.1fx)", 
    ...
    if theoretical_time is not None and theoretical_time > 0:
        ...
        logger.info("  Achieved efficiency: %.1f%% of theoretical optimum", 
                   (1/accuracy) * 100)
...
def demo_sequential_vs_jit(*, num_ops: int, delay: float) -> None:
    ...
    """
    logger.info("\n%s", "=" * 80)
    logger.info("DEMO 1: Sequential vs JIT - %d operators, %.3fs delay each", 
               num_ops, delay)
    logger.info("%s", "=" * 80)
    logger.info("Demonstrating JIT caching and optimization benefits without parallelism.")
...
def demo_explicit_parallel_execution(*, num_ops: int, delay: float) -> None:
    ...
    """
    logger.info("\n%s", "=" * 80)
    logger.info("DEMO 2: Sequential vs Parallel Execution - %d operators, %.3fs delay each", 
               num_ops, delay)
    logger.info("%s", "=" * 80)
    logger.info("Demonstrating how JIT + parallel scheduling transforms execution graphs.")
    ...
    logger.info("\nEfficiency analysis:")
    logger.info("  Sequential scheduler: %.1f%% of theoretical optimum", 
               seq_efficiency * 100)
    logger.info("  Parallel scheduler:   %.1f%% of theoretical optimum", 
               par_efficiency * 100)
...
def demo_execution_strategies(*, num_ops: int, delay: float) -> None:
    ...
    """
    logger.info("\n%s", "=" * 80)
    logger.info("DEMO 3: Execution Strategy Comparison - %d operators, %.3fs delay each", 
               num_ops, delay)
    logger.info("%s", "=" * 80)
    logger.info("Comparing different execution strategies with the same logical operations.")
    ...
    if seq_best > 0:
        ...
        if overall_speedup > 1.05:
            logger.info("\n✅ Parallelism achieved %.1fx speedup vs sequential", 
                       overall_speedup)
...
def main() -> None:
    ...
    logger.info("1. JIT tracing converts imperative code into optimizable computation graphs")
    logger.info("2. Independent operations are automatically identified for parallelization")
    logger.info("3. Execution strategies can be selected based on runtime context")
    logger.info("4. First-run tracing overhead is amortized through cached execution plans")
    logger.info("5. Context-aware operators can dynamically adapt to execution environments")

    logger.info("\nPRACTICAL APPLICATIONS:")
    logger.info("- Complex ML inference pipelines with many independent processing steps")
    logger.info("- Data transformation workflows with embarrassingly parallel operations")
    logger.info("- Ensemble methods combining multiple model predictions")
    logger.info("- Systems that need to adapt to different hardware capabilities at runtime")
