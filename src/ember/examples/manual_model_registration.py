...
def display_models():
    ...
    table = PrettyTable()
    table.field_names = ["Model ID", "Provider", "Context Window", "Input Cost", "Output Cost"]
    table.align = "l"
    ...
    for provider, ids in sorted(providers.items()):
        for model_id in sorted(ids):
            try:
                ...
                provider_name = info.provider.get("name", provider) if hasattr(info, "provider") else provider
