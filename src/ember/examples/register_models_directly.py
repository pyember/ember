...
from ember.api.models import (
    ModelInfo, ModelCost, RateLimit, register_model
)
...
def register_openai_models(api_key: str) -> List[str]:
...
def register_anthropic_models(api_key: str) -> List[str]:
...
def check_models(model_ids: List[str]) -> None:
    ...
    table = PrettyTable()
    table.field_names = ["Model ID", "Status", "Provider", "Context Window", "Input Cost", "Output Cost"]
    table.align = "l"
    ...
    for model_id in model_ids:
        ...
        if exists:
            try:
                ...
                provider_name = info.provider.get("name", "Unknown") if hasattr(info, "provider") else "Unknown"
                context_window = info.context_window if hasattr(info, "context_window") else "N/A"
                
                input_cost = f"${info.cost.input_cost_per_thousand:.4f}/1K" if hasattr(info, "cost") and info.cost else "N/A"
                output_cost = f"${info.cost.output_cost_per_thousand:.4f}/1K" if hasattr(info, "cost") and info.cost else "N/A"
...
def main():
    ...
    all_models = models.registry.list_models()
    ...
    if openai_api_key != "YOUR_OPENAI_API_KEY":
        openai_models = register_openai_models(openai_api_key)
    ...
    if anthropic_api_key != "YOUR_ANTHROPIC_API_KEY":
        anthropic_models = register_anthropic_models(anthropic_api_key)
