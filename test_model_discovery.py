...
def test_discovery_service():
    ...
    assert isinstance(discovered_models, dict), "Should return a dictionary of discovered models"
    
    # Log discovery results for debugging
    print(f"\nFound {len(discovered_models)} models via discovery")
    
    # Get provider-specific models
    openai_models = [model_id for model_id in discovered_models if model_id.startswith("openai:")]
    anthropic_models = [model_id for model_id in discovered_models if model_id.startswith("anthropic:")]
    ...
    if openai_models or anthropic_models:
        ...
        for provider, models in [("OpenAI", openai_models[:3]), ("Anthropic", anthropic_models[:3])]:
            if models:
                print(f"\n{provider} models:")
...
def test_model_invocation():
    ...
    model_registry = initialize_ember(force_discovery=True)
    ...
    assert hasattr(model_registry, 'list_models'), "Model registry should have list_models method"
    assert hasattr(model_registry, 'get_model'), "Model registry should have get_model method"
    ...
    assert model_registry.is_registered(model_id), f"Model {model_id} should be registered"
...
if __name__ == "__main__":
    ...
    # Test model invocation with actual API calls
    # This will only work if you have proper API keys set up
    print("\nTesting model invocation...")
