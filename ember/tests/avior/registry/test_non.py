"""Tests for various operators and networks in the ember framework."""

import pytest
from ember.core.non_graph import NoNGraph
from ember.registry import non
from ember.registry.dataset.dataset_registry import DatasetRegistry
from ember.registry.eval_function.eval_function_registry import (
    EvaluatorRegistry,
    MultipleChoiceEvaluator,
)
from ember.registry.model_registry.model_registry import ModelRegistry
from ember.registry.operators.operator_registry import OperatorRegistry, OperatorFactory
...
class TestVerifierOperator:
    ...
    def setup(self, setup_registries):
        ...
        # Creating a VerifierOperator instance using OperatorFactory
        unit_config = {
            "model_name": "gpt-4o",
            "temperature": 0.0,
        }  # Using a default model name
        self.verifier = OperatorFactory.create("V", unit_config)
    ...
    def test_correct_answer(self):
        ...
        correct_result, correct_answer = self.verifier.execute(correct_input)
        print(f"Correct result: {correct_result}, Correct answer: {correct_answer}")

        # Asserting verifier behember for correct answer
        assert (
            correct_result == 1
    ...
    def test_incorrect_answer(self):
        ...
        incorrect_result, incorrect_answer = self.verifier.execute(incorrect_input)
        print(
            f"Incorrect result: {incorrect_result}, Incorrect answer: {incorrect_answer}"
        )

        # Asserting verifier behember for incorrect answer
        assert (
            incorrect_result == 0
...
# Additional tests for specific operator behembers
...
class TestComplexNoNSequential:
    ...
    def setup(self, setup_registries, load_questions):
        ...
        self.complex_seq_model = NoNSequential(
            OperatorFactory.create("E", [{"model_name": "gpt-4o", "count": 5}]),
            OperatorFactory.create("SR", [{"model_name": "gpt-4o", "count": 2}]),
            OperatorFactory.create("MC", [{"model_name": "gpt-4o", "count": 1}]),
        )
    ...
    def setup(self, setup_registries, load_questions):
        ...
        class ComplexNetwork(NoNModule):
            ...
            def __init__(self):
                ...
                super().__init__()
                self.ensemble1 = non.Ensemble(num_units=5, model_name="gpt-4o")
                self.ensemble2 = non.Ensemble(num_units=3, model_name="gpt-4o")
