"""Nested NoN MCQ Experiment Example.


Usage:
    python nested_non_experiment_examples.py [--num_questions NUM] [--dataset DATASET] [--dataset_config CONFIG]

Example:
    python nested_non_experiment_examples.py --num_questions 10 --dataset mmlu --dataset_config abstract_algebra
"""
...
from ember.core.configs.config import set_config
...
from example_architectures import complex_nested_graph
...
def define_non_graph() -> NoNGraph:
    """Defining and parsing a complex nested NoN graph.

    Returns:
        An instance of NoNGraph representing the parsed complex nested graph.
    """
    logging.info("Defining and parsing complex nested NoN graph")
    non_graph = complex_nested_graph()
    logging.info("Complex nested NoN graph parsed")
    return non_graph
