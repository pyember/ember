...
from ember.core.graph_ir.operator_graph import OperatorGraph
from ember.core.graph_ir.operator_graph_runner import OperatorGraphRunner
...
from ember.core.scheduler import ExecutionPlan
from ember.registry.operator.operator_base import LMModule, Operator, OperatorMetadata, OperatorType
...
class EnsureValidChoiceOperator(Operator[EnsureValidChoiceInputs, Dict[str, Any]]):
    ...
    def forward(self, inputs: EnsureValidChoiceInputs) -> Dict[str, Any]:
        ...
        for attempt_i in range(self._max_retries):
            ...
            fallback_output: str = self.call_lm(prompt_str, self.lm_modules[0]).strip().upper()
            logging.info(
...
class MultiModelEnsemble(non.Operator[MultiModelEnsembleInputs, MultiModelEnsembleOutputs]):
    ...
    def forward(self, inputs: MultiModelEnsembleInputs) -> MultiModelEnsembleOutputs:
        ...
        judge_output = self.judge(inputs={"query": inputs.query, "responses": responses})
...
class VariedModelEnsemble(non.Operator[VariedModelEnsembleInputs, VariedModelEnsembleOutputs]):
    ...
    def __init__(self, model_configs: List[LMModuleConfig]) -> None:
        ...
        self.get_answer = non.GetAnswer(
            model_name=aggregator_model_name, temperature=max(0.0, aggregator_temp - 0.6)
        )
        self.ensure_valid_choice = EnsureValidChoiceOperator(
...
def main() -> None:
    ...
    result_table.add_row(["MultiModelEnsemble (Judge aggregator)", f"{ensemble_acc:.2f}", total_count])
