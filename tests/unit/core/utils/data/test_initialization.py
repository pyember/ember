...
class TestDatasetInitialization(unittest.TestCase):
    ...
    def test_initialize_registry(self) -> None:
        ...
        with mock.patch('ember.core.utils.data.registry.UNIFIED_REGISTRY.register_metadata') as mock_register:
            ...
            registered_datasets = {call[1]['name'] for call in mock_register.call_args_list}
            expected_datasets = {
                'truthful_qa', 'mmlu', 'commonsense_qa', 'halueval', 'my_shortanswer_ds'
            }
    ...
    def test_initialize_registry_with_real_implementation(self) -> None:
        ...
        with mock.patch('ember.core.utils.data.registry.UNIFIED_REGISTRY.register_metadata') as mock_register:
            ...
            # Get the dataset names that were registered
            registered_datasets = {call[1]['name'] for call in mock_register.call_args_list}
            expected_datasets = {'truthful_qa', 'mmlu', 'commonsense_qa', 'halueval', 'my_shortanswer_ds'}
            ...
            # Verify prepper classes were correct
            self.assertEqual(TruthfulQAPrepper, prepper_class_map['truthful_qa'])
            self.assertEqual(MMLUPrepper, prepper_class_map['mmlu'])
            self.assertEqual(CommonsenseQAPrepper, prepper_class_map['commonsense_qa'])
            self.assertEqual(HaluEvalPrepper, prepper_class_map['halueval'])
            self.assertEqual(ShortAnswerPrepper, prepper_class_map['my_shortanswer_ds'])
    ...
    def test_legacy_initialization_compatibility(self) -> None:
        ...
        with mock.patch('ember.core.utils.data.metadata_registry.UNIFIED_REGISTRY.register_metadata') as mock_register:
