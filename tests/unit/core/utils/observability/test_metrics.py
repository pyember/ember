...
from ember.core.utils.observability.metrics import METRICS, create_metrics
...
def extract_counter_value(counter, label_values):
    ...
    try:
        ...
        label_dict = {
            name: value for name, value in zip(counter._labelnames, label_values)
        }
...
def extract_histogram_count(histogram, label_values):
    ...
    try:
        # Convert label_values from tuple to dict
        label_dict = {
            name: value for name, value in zip(histogram._labelnames, label_values)
        }
...
class TestMetricsIntegration:
    ...
    def test_metrics_with_context_manager(self):
        ...
        sample_count = self._get_histogram_count(
            metrics["invocation_duration"], {"model_id": "test_model"}
        )
        assert sample_count == 1

        # Observation value should be positive (at least the sleep time)
        sample_sum = self._get_histogram_sum(
            metrics["invocation_duration"], {"model_id": "test_model"}
        )
        assert sample_sum > 0
    ...
    def test_metrics_isolation(self):
        ...
        value2 = self._get_counter_value(
            metrics2["model_invocations"], {"model_id": "test_model"}
        )
        assert value2 == 0
