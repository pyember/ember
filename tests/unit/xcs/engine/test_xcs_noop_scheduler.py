...
def test_noop_scheduler_sequential_execution() -> None:
    ...
    assert len(final_history) >= 4, f"Expected at least 4 executions, got {len(final_history)}"
    ...
    assert final_history[-1] == "node3", f"Last node executed was {final_history[-1]}, expected node3"
...
def test_noop_scheduler_wave_generation() -> None:
    ...
    assert node_a_idx < node_b_idx, f"Node A (idx {node_a_idx}) should come before Node B (idx {node_b_idx})"
    assert node_b_idx < node_c_idx, f"Node B (idx {node_b_idx}) should come before Node C (idx {node_c_idx})"
