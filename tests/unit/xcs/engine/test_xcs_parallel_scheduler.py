...
class ParallelDetectionOperator:
    ...
    def __init__(self, timeline: List[Dict[str, Any]], sleep_time: float = 0.05, name: str = "unnamed"):
...
def test_parallel_execution_detection() -> None:
    ...
    assert branch_a_data["thread"] != branch_b_data["thread"], \
...
def test_parallel_execution_with_worker_limits() -> None:
    ...
    for thread, events in thread_events.items():
        ...
        for i in range(0, len(events) - 1, 2):
            assert events[i]["event"] == "start", f"Expected start event, got {events[i]}"
            assert events[i+1]["event"] == "end", f"Expected end event, got {events[i+1]}"
            
            # Verify matching node names
            assert events[i]["node"] == events[i+1]["node"], \
