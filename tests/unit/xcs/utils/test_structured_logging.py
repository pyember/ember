...
def reset_logging_config():
    ...
    LoggingConfig.include_context_for_level = original_config["include_context_for_level"]
...
def test_time_operation_decorator(reset_logging_config):
    ...
    with patch('ember.xcs.utils.structured_logging.should_log', return_value=True), \
         patch('time.perf_counter', side_effect=[0.0, 0.1, 0.2]):  # Start time, timestamp in log, end time
    ...
    with patch('ember.xcs.utils.structured_logging.should_log', return_value=True), \
         patch('time.perf_counter', side_effect=[0.0, 0.001, 0.002]):  # Start time, timestamp in log, end time
...
def test_performance_options(reset_logging_config):
    ...
    try:
        ...
        assert LoggingConfig.enabled is False, "Enabled should be False in high performance mode"
        assert LoggingConfig.sampling_rate < 1.0, "Sampling rate should be reduced in high performance mode"
        assert LoggingConfig.default_threshold_ms > 0.0, "Threshold should be increased in high performance mode"
        assert LoggingConfig.trace_all_operations is False, "Trace all should be disabled in high performance mode"
        ...
        # Check that appropriate settings were applied
        assert LoggingConfig.enabled is True, "Enabled should be True in development mode"
        assert LoggingConfig.sampling_rate == 1.0, "Sampling rate should be 1.0 in development mode"
        assert LoggingConfig.default_threshold_ms == 0.0, "Threshold should be 0.0 in development mode"
        assert LoggingConfig.trace_all_operations is True, "Trace all should be enabled in development mode"
    finally:
