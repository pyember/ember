...
def run_with_timeout(
    func, args=None, kwargs=None, timeout_seconds=30, default_return=None
):
    """
    ...
    if success:
        ...
        for thread in threading.enumerate():
            logger.info(
                f"  - Thread {thread.name} ({thread.ident}) - Alive: {thread.is_alive()}"
            )
...
def debug_model_discovery_provider(provider_class, provider_name):
    ...
    try:
        ...
        models, duration, success = run_with_timeout(
            provider_instance.fetch_models, timeout_seconds=20
        )
        ...
        print(
            f"✅ Successfully fetched {len(models)} {provider_name} models in {duration:.2f}s"
        )
...
def debug_discovery_service():
    ...
    discovered_models, duration, success = run_with_timeout(
        discovery_service.discover_models, timeout_seconds=30
    )
    ...
    print(
        f"✅ Successfully discovered {len(discovered_models)} models in {duration:.2f}s"
    )
